...
將作者與 AI 討論留下的寫作痕跡移出論文正文，保留真正的研究內容、證據與限制。適用於不同語言的稿件，撰寫語言由使用者決定；正文、圖說與圖中文字可依需求採用不同語言。
...
流程涵蓋向量／點陣圖層區分、素材來源、渲染檢查、使用者指定語言的短圖說、正文介紹及LaTeX自動編號。一般文字修訂不自動啟動圖像生成或改動版面；圖片生成與PDF輸出依所在環境的工具與相關skill執行。
